...
async def create_repair_request(payload: RepairRequestCreate, user: dict = Depends(require_user)):
    ...
    created = await civic_repo.insert_one("repair_requests", request)
    await record_audit_event("repair_request", created["request_id"], "created", user, after={"complaint_id": payload.complaint_id, "estimated_price": payload.estimated_price})
...
async def decide_repair_request(request_id: str, payload: DecisionCreate, _admin: dict = Depends(require_admin)):
    ...
    updated = await civic_repo.update_one("repair_requests", "request_id", request_id, {
        ...
        "funds_status": "Demo Budget Reserved" if payload.approved else "Not Allocated",
        "admin_note": payload.note,
    })
    await record_audit_event("repair_request", request_id, "approved" if payload.approved else "rejected", _admin, {"status": request.get("status")}, {"status": updated.get("status"), "approved_budget": updated.get("approved_budget")}, payload.note)
    return updated
...
async def submit_repair_proof(request_id: str, payload: ProofCreate, user: dict = Depends(require_user)):
    ...
    await record_audit_event("repair_request", request_id, "proof_submitted", user, {"status": request.get("status")}, {"status": updated.get("status")}, payload.completion_note)
    return updated
...
async def release_funds(request_id: str, _admin: dict = Depends(require_admin)):
    ...
    updated = await civic_repo.update_one("repair_requests", "request_id", request_id, {
        "status": "Completed",
        "funds_status": "Payment Approved (Demo)",
        "funds_released_at": now_utc(),
    })
